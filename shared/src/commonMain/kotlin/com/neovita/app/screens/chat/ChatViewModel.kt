...
class ChatViewModel(private val chatRepo: ChatRepository) {
    ...
    fun sendMessage(text: String) {
        ...
        val userMsg = ChatMessage(uuid(), MessageRole.USER, text, Clock.System.now().toEpochMilliseconds())
        val assistantMsg = ChatMessage(uuid(), MessageRole.ASSISTANT, "", Clock.System.now().toEpochMilliseconds())
        _state.update {
            it.copy(
    ...
    private fun uuid() = "msg-${Clock.System.now().toEpochMilliseconds()}-${(0..9999).random()}"
}
